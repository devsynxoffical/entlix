  ...
  return (
    <>
      <div 
        ...
        className="glass-card overflow-hidden flex flex-col justify-between border border-slate-200/80 hover:border-purple-300 transition-all shadow-sm hover:shadow-md cursor-pointer group relative"
      >
        <div>
          {/* Header bar */}
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            >
              ...
              <span className="text-xs text-slate-900 font-bold truncate max-w-[130px] group-hover:text-purple-600 transition-colors">
                {ad.advertiserName}
              </span>
          ...
          <div className="p-5">
            {ad.adCreativeUrl ? (
              <div className="w-full h-44 bg-slate-100 rounded-xl mb-4 flex items-center justify-center overflow-hidden border border-slate-200/60 relative">
                ...
                <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <span className="bg-white/95 text-slate-900 text-xs font-bold px-3.5 py-2 rounded-xl flex items-center gap-2 shadow-lg">
            ...
            {ad.advertiserLink && (
              <div className="mb-3 flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-slate-100 px-3 py-1 rounded-lg w-fit border border-slate-200/60">
        ...
        {/* Footer Metadata */}
        <div className="px-5 pb-4 pt-3 bg-slate-50/40 border-t border-slate-100 grid grid-cols-2 gap-y-2 gap-x-4 text-[11px] font-medium text-slate-500">
      ...
      {/* Ultra-Clean Ad Details & AI Modal */}
      {showModal && (
        <div 
          onClick={() => setShowModal(false)}
          className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fade-in"
        >
          <div 
            onClick={(e) => e.stopPropagation()}
            className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[88vh]"
          >
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/80 flex flex-col gap-3">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  {ad.advertiserLogo ? (
                    <img src={ad.advertiserLogo} alt="Logo" className="w-9 h-9 rounded-full object-cover border border-slate-200 shadow-sm" />
                  ) : (
              ...
              <div className="flex gap-2 border-t border-slate-200/60 pt-3">
                <button
                  ...
                  className={`text-xs font-bold px-3.5 py-1.5 rounded-xl transition-all ${activeTab === 'DETAILS' ? 'bg-purple-600 text-white shadow-md shadow-purple-500/20' : 'text-slate-600 hover:bg-slate-200/60'}`}
                >
                  Ad Creative & Copy
                </button>
                <button
                  ...
                  className={`text-xs font-bold px-3.5 py-1.5 rounded-xl transition-all flex items-center gap-1.5 ${activeTab === 'AI_COUNTER' ? 'bg-purple-600 text-white shadow-md shadow-purple-500/20' : 'text-purple-700 bg-purple-50 hover:bg-purple-100'}`}
                >
                  <Sparkles size={14} />
            ...
            <div className="p-6 overflow-y-auto flex flex-col gap-5">
              {activeTab === 'DETAILS' ? (
                <>
                  {ad.adCreativeUrl && (
                    <div className="w-full max-h-72 bg-slate-900 rounded-2xl overflow-hidden flex items-center justify-center border border-slate-200/80 shadow-md">
                      <img src={ad.adCreativeUrl} alt="Ad Creative Preview" className="max-w-full max-h-72 object-contain" />
                    </div>
                  )}

                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Ad Copy & Description</h4>
                    <div className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl text-sm leading-relaxed text-slate-800 whitespace-pre-wrap font-normal">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs font-semibold">
                    ...
                    <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-700 flex flex-col gap-1 col-span-2 sm:col-span-1">
                      ...
                      <span className="font-bold flex items-center gap-1"><Clock size={12} /> {new Date(ad.firstDetectedAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                </>
              ) : (
                      ...
                      <div>
                        ...
                        <ul className="space-y-1.5">
                          {aiAnalysis.targetPainPoints.map((pt: string, idx: number) => (
                            <li key={idx} className="text-xs text-slate-700 font-semibold bg-slate-50 p-3 rounded-xl border border-slate-200/60 flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-4">
                          {aiAnalysis.counterAds.map((cAd: any, i: number) => (
                            <div key={i} className="p-4 bg-slate-900 text-white rounded-2xl border border-slate-800 flex flex-col gap-2 shadow-md">
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-800 flex justify-between items-center text-xs">
                                ...
                                <button
                                  ...
                                  className="text-purple-400 hover:text-purple-300 font-bold flex items-center gap-1 bg-purple-950/60 px-2.5 py-1 rounded-lg border border-purple-800"
                                >
            ...
            {/* Modal Footer */}
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/80 flex flex-col sm:flex-row justify-between items-center gap-3">
              <button
                onClick={copyLink}
                disabled={!ad.sourceLink}
                className="btn btn-secondary text-xs py-2.5 px-4 gap-1.5 w-full sm:w-auto font-bold"
              >
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                {whatsappUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn bg-emerald-600 hover:bg-emerald-700 text-white text-xs py-2.5 px-4 gap-1.5 font-bold shadow-md shadow-emerald-500/20"
                  >
                    <MessageSquare size={14} />
                ...
                {ad.sourceLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn btn-primary text-xs py-2.5 px-5 gap-2 shadow-lg shadow-purple-500/20 font-bold"
                  >
                    <span>Open Live Meta Ad</span>
                ...
                <>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="p-3.5 bg-purple-50 border border-purple-100 rounded-2xl">
                      <span className="text-2xl font-black text-purple-700">{dossierData.totalAdsTracked}</span>
                    ...
                    <div className="p-3.5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                      <span className="text-2xl font-black text-emerald-700">{dossierData.newAdsCount}</span>
                      <p className="text-[10px] font-bold text-slate-500 uppercase mt-0.5">New Ads</p>
                    </div>
                    <div className="p-3.5 bg-cyan-50 border border-cyan-100 rounded-2xl">
                      <span className="text-2xl font-black text-cyan-700">{dossierData.whatsappCount}</span>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {dossierData.keywordsUsed.map((kw: string, i: number) => (
                        <span key={i} className="text-xs bg-slate-100 text-slate-700 px-2.5 py-1 rounded-xl font-bold">#{kw}</span>
                      ))}
                      {dossierData.regionsTargeted.map((rg: string, i: number) => (
                        <span key={i} className="text-xs bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded-xl font-bold">📍 {rg}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-3">
                      {dossierData.ads.map((portfolioAd: any) => (
                        <div key={portfolioAd.id} className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-2xl flex items-center justify-between gap-3 text-xs">
