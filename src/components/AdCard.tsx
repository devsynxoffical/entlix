...
import { ExternalLink, Tag, MapPin, Copy, Star, MessageSquare, Building2, Eye, Globe } from 'lucide-react';
    ...
    >
      <div>
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/60">
          ...
          >
            {ad.advertiserLogo ? (
              <img src={ad.advertiserLogo} alt="Logo" className="w-8 h-8 rounded-full object-cover border border-slate-200 shrink-0 shadow-xs" />
            ) : (
            ...
            <span className={`badge shrink-0 ml-1 ${isNew ? 'badge-new' : 'badge-existing'}`}>
              {ad.classification}
            </span>
          </div>
        ...
        <div className="p-4 pb-2">
          {ad.adCreativeUrl ? (
            <div className="w-full h-48 bg-slate-100 rounded-xl mb-3 flex items-center justify-center overflow-hidden border border-slate-200/60 relative">
              <img 
            ...
            <div className="w-full h-36 bg-slate-50 rounded-xl mb-3 flex flex-col items-center justify-center border border-dashed border-slate-200">
              <span className="text-slate-400 text-xs font-medium">Meta Ad Creative</span>
            </div>
          )}
          ...
          {ad.advertiserLink && (
            <div className="mb-2.5 flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-slate-100 px-3 py-1 rounded-lg w-fit border border-slate-200/60">
